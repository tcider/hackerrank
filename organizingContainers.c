...
void    null_arr(int size, int *arr)
{
    int i = -1;
    while (++i < size)
        arr[i] = 0;
...
char* organizingContainers(int container_rows, int container_columns, int** container) {
    ...
    if (container_columns != container_rows)
    return (imp);
    rows = (int*)malloc(sizeof(int) * container_rows);
    cols = (int*)malloc(sizeof(int) * container_columns);
    null_arr(container_rows, rows); 
    null_arr(container_columns, cols);
    i = -1;
    while (++i < container_rows)
    {
        j = -1;
        while (++j < container_columns)
        {
            rows[i] += container[i][j];
            cols[j] += container[i][j];
        }            
    }
    sort_arr(container_rows, rows);
    sort_arr(container_columns, cols);
    i = -1;
    while (++i < container_rows)
        if (rows[i] - cols[i])
            return (imp);
